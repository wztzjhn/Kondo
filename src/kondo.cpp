...
void read_state_from_dump(fs::path const& dump_dir, fkpm::RNG &rng, Model &m, Dynamics &dynamics) {
    Vec<fs::path> v;
    fs::path p(dump_dir);
    ...
    if (v.size() > 1) {
        auto num_lines = [] (const fs::path &p) {
            std::ifstream file(p);
...
int main(int argc, char *argv[]) {
    ...
    auto dump = [&](int step) {
        ...
        std::ofstream dump_file(base_dir/"dump"/fname.str());
    ...
    if (!fs::is_directory(base_dir/"dump")) {
        fs::create_directory(base_dir/"dump");
    }
    fs::directory_iterator it(base_dir/"dump"), end_it;
