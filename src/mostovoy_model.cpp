...
void MostovoyModel::set_hamiltonian(Vec<vec3> const& spin) {
    ...
    for (int i = 0; i < n_sites; i++) {
        ...
        // On-site p term
        cx_flt delta_cx = {static_cast<float>(delta), 0.0};
        for (int b = 0; b < 3; b++) {
            for (int s = 0; s < 2; s++) {
        ...
        for (int a = 0; a < 2; a++) {
            for (int b = 0; b < 6; b++) {
                cx_flt t = {static_cast<float>(t_pds * t_alpha_b[a][b % 3]), 0.0};
                for (int s = 0; s < 2; s++) {
        ...
        // p-p hopping
        cx_flt t_pp_cx  = {static_cast<float>(t_pp), 0.0};
        for (int b = 0; b < 6; b++) {
            for (int c = 0; c < 6; c++) {
                if (b % 3 == c % 3) continue;
                for (int s = 0; s < 2; s++) {
